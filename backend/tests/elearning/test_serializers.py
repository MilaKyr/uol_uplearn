...
def test_create_feedback(student, course, enrolled_student):
    ...
    result = CourseCreateFeedback(context={"request": request}).create({"course_id": course.id, "rating": 3})
...
def test_create_feedback_fails(student, course):
    ...
    with pytest.raises(PermissionDenied):
        CourseCreateFeedback(context={"request": request}
