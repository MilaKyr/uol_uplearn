...
def test_list_courses(student_group, active_course):
    ...
    response = client.get(
        "/api/courses/", headers={"AUTHORIZATION": f" Bearer {student['access']}"}
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_get_course(student_group, active_course):
    student = register_student()
    response = client.get(
        f"/api/courses/{active_course.id}/",
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_users_found(teacher_group, student):
    teacher = register_teacher()
    response = client.get(
        f"/api/dashboard/{teacher['user']['id']}",
        headers={"AUTHORIZATION": f" Bearer {teacher['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_todo(student_group):
    student = register_student()
    response = client.get(
        "/api/todo_for?month=2&year=2025",
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_todo_with_week(student_group):
    student = register_student()
    response = client.get(
        "/api/todo_for?month=2&year=2025&week=8",
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_lesson_done(student_group, course, lesson):
    ...
    response = client.patch(
        f"/api/lessons/progress/{lesson.id}/",
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_create_feedback_fails(student_group, course):
    ...
    enrollment.status = "finished"
    enrollment.save()
    payload = {"course_id": str(course.id), "text": "test feedback", "rating": -1}
    response = client.post(
        "/api/feedbacks/",
        data=payload,
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
...
def test_course_search_all(student_group, active_course):
    student = register_student()
    response = client.get(
        "/api/courses/search", headers={"AUTHORIZATION": f" Bearer {student['access']}"}
    )
    assert response.status_code == status.HTTP_200_OK
    result = json.loads(response.content)
    assert len(result) == 1
...
def test_course_search_title(student_group, active_course):
    student = register_student()
    response = client.get(
        f"/api/courses/search?query={active_course.title[:3]}",
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
    result = json.loads(response.content)
...
def test_course_search_tag(student_group, tag, active_course):
    student = register_student()
    response = client.get(
        f"/api/courses/search?tag={tag.name}",
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
    result = json.loads(response.content)
...
def test_course_search_tag_with_title_none(student_group, tag, active_course):
    student = register_student()
    response = client.get(
        f"/api/courses/search?tag={tag.name}&query={active_course.title}",
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
    result = json.loads(response.content)
...
def test_course_search_tag_with_title_found(student_group, tag, active_course_tag):
    student = register_student()
    response = client.get(
        f"/api/courses/search?tag={tag.name}&query={active_course_tag.title}",
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
    result = json.loads(response.content)
...
def test_get_new_course_id(teacher_group, lesson):
    teacher = register_teacher()
    response = client.get(
        f"/api/new_course_id", headers={"AUTHORIZATION": f" Bearer {teacher['access']}"}
    )
    assert response.status_code == status.HTTP_201_CREATED


@pytest.mark.django_db
def test_get_study_course_teacher_works(teacher_group, active_course):
    teacher = register_teacher()
...
@pytest.mark.django_db
def test_get_study_course_student_done_works(
    student_group, active_course, active_topic, active_lesson
):
    student = register_student()
    enroll(student["access"], f"{active_course.id}")
    response = client.patch(
        f"/api/lessons/progress/{active_lesson.id}/",
        headers={"AUTHORIZATION": f" Bearer {student['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
    response = client.get(
...
def test_enrolled_students(teacher_group, active_course):
    teacher = register_teacher()
    response = client.get(
        f"/api/enrollments/students?course_id{active_course.id}",
        headers={"AUTHORIZATION": f" Bearer {teacher['access']}"},
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_user_search_all(teacher_group, student_group):
    ...
    student = register_student()
    response = client.get(
        f"/api/users/search", headers={"AUTHORIZATION": f" Bearer {teacher1['access']}"}
    )
    result = json.loads(response.content)
    assert len(result) == 2
    assert [user["id"] for user in result].sort() == [
        student["user"]["id"],
        teacher2["user"]["id"],
    ].sort()
...
def test_user_search_students(teacher_group, student_group):
    ...
    student = register_student()
    response = client.get(
        f"/api/users/search?role=Student",
...
def test_user_search_by_status(teacher_group, student_group, course):
    ...
    enroll(student["access"], course["id"])
    response = client.get(
        f"/api/users/search?status=started",
        headers={"AUTHORIZATION": f" Bearer {teacher1['access']}"},
    )
    result = json.loads(response.content)
    assert len(result) == 1
...
def test_user_search_by_name(teacher_group, student_group):
    ...
    response = client.get(
        f"/api/users/search?query={student_name[0]}",
        headers={"AUTHORIZATION": f" Bearer {teacher1['access']}"},
    )
    result = json.loads(response.content)
    assert len(result) == 1
...
def test_user_search_by_course(teacher_group, student_group):
    ...
    _ = register_student()
    response = client.get(
        f"/api/users/search?course_id={course['id']}",
        headers={"AUTHORIZATION": f" Bearer {teacher1['access']}"},
    )
    result = json.loads(response.content)
    assert len(result) == 0
