...
def test_update_enrollment_status(teacher_group, student_group):
    ...
    assert Notification.objects.filter(person__id=student['user']['id'],
                                       recipient__id=teacher['user']['id'], course__id=course['id']).exists()

    client.patch(f"/api/enrollments/{enrollment.id}/", data={"status": "blocked"},
