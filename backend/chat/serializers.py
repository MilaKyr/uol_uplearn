...
class ConversationSerializer(serializers.ModelSerializer):
    ...
    def get_last_message(self, instance):
        last_message = (
            Message.objects.filter(conversation=instance).order_by("created").last()
        )
    ...
    def get_unread_messages_ids(self, instance):
        user = self.context["request"].user
        messages = instance.messages.filter(seen=False, recipient=user).values_list(
            "id"
        )
...
class ConversationCreateSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        conversation = Conversation.objects.filter(users__in=[sender]).filter(
            users__in=[recipient]
        )
