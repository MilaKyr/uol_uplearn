...
from .models import Course, User, Topic, Lesson, Feedback, CourseEnrollment, Tag, Student, Teacher
from .serializers import (
    ...
    CourseRetireveUpdateSerializer,
    LessonEditFilesSerializer,
    LessonWithFilesCheckSerializer,
    CourseOwnerSerializer,
    CourseTitleSerializer,
    UserAuthSerializer,
    PrefetchEnrollmentSerializer,
    CourseBasicSerializer,
    LessonBasicSerializer,
    TeacherSettingsSerializer,
    StudentSettingsSerializer,
    EnrollmentCreateSerializer,
    EnrollmentUpdateSerializer,
    CoursePhotoSerializer, SettingsSerializer,
)
...
class CourseListCreateView(generics.ListCreateAPIView):
    ...
    queryset = (
        ...
        .annotate(
            average_rating=Avg(
                "registered_students__feedback__rating",
                default=0,
        ...
        .annotate(
            n_students=Count(
                "registered_students__feedback__id",
    ...
    def get_queryset(self):
        return self.queryset.exclude(registered_students__student__user=self.request.user)
...
class CourseSearch(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if query_exists and tag_exists:
            return queryset.prefetch_related("tags").filter(
                Q(title__iregex=query) & Q(tags__name__iexact=tag)
            )
        if query_exists:
    ...
    def get_serializer_context(self):
        ...
        if self.request and self.request.user.is_student():
            enrolled = CourseEnrollment.objects.filter(
                student__user=self.request.user, course__id=self.kwargs["pk"]
            )
...
class CourseStudyDetail(generics.RetrieveAPIView):
    ...
    queryset = (
        Course.objects.prefetch_related("registered_students__feedback")
        .annotate(
            average_rating=Avg(
                "registered_students__feedback__rating",
                default=0,
    ...
    serializer_class = CourseSerializer
    permission_classes = [permissions.IsAuthenticated, OwnerOrEnrolled, HasAccess]

    def get_queryset(self):
        # if student, filter only active courses
...
class EnrolledStudentsView(generics.ListAPIView):
    ...
    queryset = CourseEnrollment.objects.filter(
        status__in=["started", "blocked", "removed"]
    )
...
class TodoListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        first_date, last_date = self._get_dates()
            ...
            CourseEnrollment.objects.prefetch_related("done_lessons")
            .filter(student__user=self.request.user, done_lessons__id__isnull=False)
            .values_list("done_lessons__id")
            .distinct()
        )
        queryset = (
            self.queryset.filter(course__registered_students__student__user=self.request.user)
...
class TopicView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def _get_lessons(self, topic):
        lessons = Lesson.objects.prefetch_related("students").filter(topic=topic).all()
...
class LessonRetrieveDestroyUpdateView(generics.RetrieveUpdateDestroyAPIView):
    ...
    queryset = (
        Lesson.objects.prefetch_related("topic")
...
class CourseProgressUpdateView(generics.UpdateAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated, IsStudent, IsEnrolled, HasAccess]
...
class FeedbackListCreateView(generics.ListCreateAPIView):
    ...
    queryset = (
        CourseEnrollment.objects.prefetch_related("feedback")
    ...
    permission_classes = [
        permissions.IsAuthenticated,
        FinishedCourse,
    ]  # checks if user finished the course in serializer
...
class UserRetrieveView(generics.RetrieveAPIView):
    ...
    def _get_student_courses(self, user, is_owner):
        queryset = (
            self._get_course_queryset()
            .prefetch_related("topics")
            .prefetch_related("topics__lessons")
            .distinct()
            .annotate(
                overall=Count(
                    "topics__lessons", output_field=IntegerField(), distinct=True
                )
            )
        )
        if is_owner:
            return queryset.filter(registered_students__student__user=user, is_active=True).all()
        return queryset.filter(
            registered_students__status="started",
            registered_students__student__user=user,
            is_active=True,
        ).all()
    ...
    def _get_courses_info(self, user, is_owner):
        if user.is_student():
            ...
            if not is_owner:
                ...
                CourseEnrollment.objects.select_related("done_lessons")
                .filter(student__user=self.request.user)
                .annotate(
                    done=Count(
                        "done_lessons__id", distinct=True, output_field=IntegerField()
                    )
                )
                .values_list("course__id", "student__user__id", "created", "done")
                .all()
            )
            courses_serializer = CourseWithProgressSerializer(
                courses, many=True, context={"done": done}
            )
            data = sorted(
                courses_serializer.data, key=lambda x: x["status"], reverse=True
            )
            return data
    ...
    def _get_todo_for(self, month, year, tzinfo):
        ...
        todo = (
            ...
            .prefetch_related("students")
            .filter(students__student__user=self.request.user)
            .annotate(n_done=Count("students__id", distinct=True))
            .filter(
                deadline__range=(first_date, last_date),
                n_done=0,
                topic__course__registered_students__student__user=self.request.user,
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        response = self.get_base_data(user)
...
class SettingsProfileSerializer(generics.RetrieveUpdateAPIView):
...
class UserSearchListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        course_exists = (
            course_id is not None and course_id != "" and course_id != "null"
        )
        ...
        if query_exists:
            ...
            queryset = queryset.filter(
                Q(first_name__iregex=query_regex) | Q(last_name__iregex=query_regex)
            )
        if role_exists:
            role = role.lower().strip()
            queryset = queryset.prefetch_related("groups").filter(
                groups__name__iregex=role
            )
        if status_exists:
            status_list = [stat.lower() for stat in status.strip().split(",")]
            status_regex = r"({})".format("|".join(status_list))
            queryset = queryset.prefetch_related("student_profile__enrollment").filter(
                student_profile__enrollment__status__iregex=status_regex,
                student_profile__enrollment__course__teacher__user=self.request.user,
            )
        if course_exists:
            queryset = queryset.prefetch_related("student_profile__enrollment").filter(
                student_profile__enrollment__course__id=course_id,
                student_profile__enrollment__course__teacher__user=self.request.user,
            )
